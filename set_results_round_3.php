<?php
	...
	{	
		...
		if($conn)
		{	
			$n=mysqli_fetch_array(mysqli_query($conn,"select max(serial) from round_3 "))[0];
			for($i=1;$i<=$n;$i++)
			{	
				$winid=$_POST["match".$i];
				$sql="select * from round_3 where serial='" . $i . "' ";
				...
				$sql="update round_3 set winner='" . $winid . "' where serial='" . $i . "' ";
				...
				$score=$row['score']+10;
				$wins=$row['wins']+1;
				$sql="update player_list set round_3=10, wins='" . $wins . "', score='" . $score . "' where player_id='" . $winid . "'";
				if(mysqli_query($conn,$sql))
				...
				$score=$row['score']-10;
				$loses=$row['loses']+1;
				$sql="update player_list set round_3=-10, loses='" . $loses . "', score='" . $score . "' where player_id='" . $loseid . "'";
				if(mysqli_query($conn,$sql))
